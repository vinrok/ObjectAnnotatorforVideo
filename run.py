...
# credits to zhangzjn's github repo regarding video annotation.
...
from libs.lib import newAction, addActions, x_y_w_h
from libs.pascal_voc_io import PascalVocWriter
from libs.video_play import VideoPlay
from tracking.kcftracker import *

__appname__ = 'Annotate Object in VideoFile'
...
class MainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        label_save_gap = QLabel(u'save gap')
    ...
    def load_video(self, filepath=None):
        if filepath is not None:
            ...
            if self.cap.isOpened():
                ...
                self.video_width = self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1366)
                self.video_height = self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 768)
    ...
    def event_button_tracking(self):
        if self.event_button_save_current():
            ...
            for i in range(trackerCount):
                ...
                x, y, w, h = x_y_w_h(firstLastPointList[i], firstEndPointList[i])
    ...
    def event_button_save(self):
        ...
        if (self.check_state_change_value):
            saveImagePath = os.path.join(self.save_dir_path, 'annotatedObjects')
            saveAnnosPath = os.path.join(self.save_dir_path, 'coordsAnnotations')
            ...
            for num in range(self.save_start_frame, self.save_end_frame + 1, self.save_gap):
                if(num in self.labelled_frame_dict.keys()):
                    ...
                    for i in range(tempCount):
                        x, y, w, h = x_y_w_h(temptLastPointList[i], tempEndPointList[i])
